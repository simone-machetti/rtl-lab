...
class Mapping1:
    ...
    def apply_patch(basedir, patchfile):
        # Apply with the target directory as CWD instead of `--directory`, which
        # git resolves relative to an enclosing repo root (and which blocks some
        # renames). GIT_CEILING_DIRECTORIES stops git's repo discovery so it
        # treats the files as plain -- the repo .git is never moved (no fragile
        # `mv ../../.git`). Absolute paths make this independent of the caller's
        # working directory.
        basedir = Path(basedir).resolve()
        patchfile = Path(patchfile).resolve()
        cmd = ['git', 'apply', '-p1', str(patchfile)]
        if verbose:
            cmd += ['--verbose']
        env = dict(os.environ)
        env['GIT_CEILING_DIRECTORIES'] = str(basedir.parent)
